...
function varargout = ECM_fiber_graph(varargin)
% ECM_FIBER_GRAPH MATLAB code for ECM_fiber_graph.fig
%      ECM_FIBER_GRAPH, by itself, creates a new ECM_FIBER_GRAPH or raises the existing
%      singleton*.
%
%      H = ECM_FIBER_GRAPH returns the handle to a new ECM_FIBER_GRAPH or the handle to
%      the existing singleton*.
%
%      ECM_FIBER_GRAPH('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ECM_FIBER_GRAPH.M with the given input arguments.
%
%      ECM_FIBER_GRAPH('Property','Value',...) creates a new ECM_FIBER_GRAPH or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before ECM_fiber_graph_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to ECM_fiber_graph_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help ECM_fiber_graph

% Last Modified by GUIDE v2.5 07-Jun-2022 11:00:00

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @ECM_fiber_graph_OpeningFcn, ...
                   'gui_OutputFcn',  @ECM_fiber_graph_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before ECM_fiber_graph is made visible.
function ECM_fiber_graph_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to ECM_fiber_graph (see VARARGIN)
% global startingFolder
% Choose default command line output for ECM_fiber_graph
...
% UIWAIT makes ECM_fiber_graph wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = ECM_fiber_graph_OutputFcn(hObject, eventdata, handles) 
...
%%%%define parameters for morphological skeletonization
Radius                       = 30;

[ExtremNode, SkeletonMaxGabor,~,~,~] = getMorphoSkelGraph(pgmFileName,draw_graph, BinaryImage);   
...
if sum(sum(MaxGabor)) == 0 
    ...
    wb = waitbar(0, 'Reconnection of missing fibers ...','WindowStyle', 'modal');
        wbch = allchild(wb);
        jp = wbch(1).JavaPeer;
        jp.setIndeterminate(1);
...
idx = [];
for countFiles = 1: length(MyListOfFiles)
    pos = strfind(MyListOfFiles{countFiles},'.');
    file_ext = MyListOfFiles{countFiles}(pos+1:end);
    if ~strcmp(file_ext, 'tif') && ~strcmp(file_ext, 'jpg') && ~strcmp(file_ext, 'png')... 
       && ~strcmp(file_ext, 'jpeg') && ~strcmp(file_ext, 'tiff') 
        idx  = [idx countFiles];
    end
end
...
message = sprintf('Would you like the add skeleton reconnection?');
button = questdlg(message,'Skeleton Reconnection', 'Yes', 'No', 'Yes');
drawnow;  % Refresh screen to get rid of dialog box remnants.
...
parfor nb_img = 1: length(MyListOfFiles)
    ...
    save_dir = './stats/img/'; 
    mkdir(save_dir,MyListOfFiles{nb_img});
    ...
    if bGround
        save_namefile = [fullfile(save_dir, MyListOfFiles{nb_img}, 'Gabor.png')];
        imwrite((MaxGabor), save_namefile, 'png');
    else
        save_namefile = [fullfile(save_dir, pgmFileName, 'Gabor.png')];
        imwrite(imcomplement(MaxGabor), save_namefile, 'png');
    ...
    [ExtremNode, SkeletonMaxGabor,~,~,~] = getMorphoSkelGraph(MyListOfFiles{nb_img},draw_graph, BinaryImage); 
    
    save_namefile = [fullfile(save_dir, MyListOfFiles{nb_img}, 'MorphologicalSkeleton.png')];
    imwrite((imcomplement(SkeletonMaxGabor)), save_namefile, 'png');
    
    if recon_link
        ThreshOnSkeletonLengthBranch = 5;
        Radius                       = 30;
        [ ~,FinalSkeleton ] = ConnectMissingFibers(MyListOfFiles{nb_img}, MaxGabor, imageSample, SkeletonMaxGabor,...
            ExtremNode,ThetaMaxGabor, Radius, draw_graph, ThreshOnSkeletonLengthBranch );
        SkeletonMaxGabor = FinalSkeleton;    
    end
    ...
    [DeformMap, stats_edges] = getLengthMap(G,distanceGraph,y,rows,cols,LambdaMaxGabor, ThetaMaxGabor);
...
if sum(sum(G)) == 0 
    ...
    [rows, cols] = size(imageData);
    distanceGraph = DistanceLength(G,y,'INT',1);     
    [DeformMap, ~] = getLengthMap(G,distanceGraph,y,rows,cols,LambdaMaxGabor, ThetaMaxGabor);
    DeformMap = inpaint_nans(DeformMap,4);
    DeformMap = imgaussfilt(DeformMap,7,'Padding','replicate');
...
if sum(sum(G)) == 0 
    ...
    [rows, cols] = size(imageData);
    distanceGraph = DistanceLength(G,y,'INT',1);  
    [DeformMap_1, stats_edges] = getLengthMap(G,distanceGraph,y,rows,cols,LambdaMaxGabor, ThetaMaxGabor);

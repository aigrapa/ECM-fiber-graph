function [ MatrixA] = FillCostMatrixAngleConstraint(RowsDisk,ColsDisk, Energy,Angle,DilatedSkelWExtrem, linearIndLastPoint, AngleStartPoint)
                            ...
                            for SignX = [-1,0, 1] % go through the whole 8 neighbours
                                for SignY = [-1,0, 1]                                    
                                        ...
                                        if Row_point_end > 0 && Row_point_end <= size(Energy,1) && Col_point_end > 0 && Col_point_end <= size(Energy,2) %%check that I'm within borders
                                            ...
                                            %weight1 = abs(Angle(RowsDisk(index_neighbour),ColsDisk(index_neighbour))-Angle(RowsDisk(j),ColsDisk(j)))+0.001;
                                            %weight1 = abs(Angle(RowsDisk(index_neighbour),ColsDisk(index_neighbour))-AngleStartPoint)+0.001;

                                            %weight = weight + 1/3*weight1;

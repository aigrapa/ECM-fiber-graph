function [ExtremNode, SkeletonMaxGabor,g,node,link] = getMorphoSkelGraph(pgmFileName,draw_graph,bw)

%getMorphoSkelGraph computes and draws the 2D morphological skeleton (SkeletonMaxGabor)
%with an associated network graph
%The input (bw) binary image is a binarized image of previously detected fibers, 
%and graph g is the adjacency or link  matrix that indicates the nodes
%that are connected, matrix entries represent the link
...
%ExtremNode holds the coordinates of the extreme nodes in the morphological 
%graph skeleton 
...
SkeletonMaxGabor = Graph2Skel3D(node,link,x_length,y_length,1);
[graph2,node2,link2] = Skel2Graph3D(SkeletonMaxGabor,0);
